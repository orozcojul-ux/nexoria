...
import { translateItem, translateRarity } from "@/lib/translate-game";
...
import {
  getInventoryTabGuide,
  getInventoryTabGuides,
  getInventoryActionsGuide,
  getInventorySourcesGuide,
} from "@/lib/inventory-guide-i18n";
import { relicUsageInfo, shopOwnedUsageInfo } from "@/lib/itemUsageHelp";
...
export default function Inventory() {
  ...
  useInventorySync(useCallback((detail) => {
    ...
    if (detail?.source === "shop" && detail?.name) {
      toast.success(t("inventory.shop.added", { name: detail.name }));
    }
  ...
  const equipCosmetic = async (sku, slot) => {
    try {
      ...
      sfx.success();
      toast.success(slot === "frame" ? t("inventory.equip.frame") : t("inventory.equip.banner"));
      await refresh();
    } catch (e) {
      toast.error(translateApiError(t, e, "inventory.equip.failed"));
    }
  };
  ...
  const equipAura = async (sku) => {
    try {
      ...
      sfx.success();
      toast.success(t("inventory.equip.aura"));
      await refresh();
    } catch (e) {
      toast.error(translateApiError(t, e, "inventory.equip.failed"));
    }
  ...
  const equipMount = async (sku) => {
    try {
      ...
      sfx.success();
      toast.success(t("inventory.equip.mount"));
      await refresh();
    } catch (e) {
      toast.error(translateApiError(t, e, "inventory.equip.failed"));
    }
  ...
  const openChest = async () => {
    ...
    try {
      ...
      if (!data.items || data.items.length === 0) {
        toast.info(t("inventory.chest.duplicates", { count: data.refunded || 50 }));
      } else {
        setNewItems(data.items);
      }
      await load(); await refresh();
    } catch (e) { toast.error(translateApiError(t, e, "inventory.chest.failed")); }
  ...
  const dedupe = async () => {
    try {
      ...
      if (data.removed > 0) {
        toast.success(t("inventory.compact.ok", { count: data.removed }));
        sfx.success();
      } else {
        toast.info(t("inventory.compact.none"));
      }
      await load();
    } catch (e) { toast.error(t("inventory.compact.failed")); }
  };

  const afterEconomy = async () => { await load(); await loadTrades(); await refresh(); };

  const localizedItems = useMemo(() => items.map((it) => translateItem(t, it)), [items, t]);
  const localizedRarities = useMemo(
    () => rarities.map((r) => ({ ...r, name: translateRarity(t, r.id) || r.name })),
    [rarities, t],
  );

  const filtered = filter === "all" ? localizedItems : localizedItems.filter((i) => i.rarity === filter);
  const banner = usePageBanner("inventory", { count: localizedItems.length });
  const incomingCount = trades.incoming?.length || 0;
  const tabGuide = useMemo(() => getInventoryTabGuide(t, tab), [t, tab]);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4 justify-center" data-testid="inventory-tabs">
        {[
          { id: "relics", label: t("inventory.tab.relics"), icon: Gem, count: localizedItems.length },
          { id: "cosmetics", label: t("inventory.tab.cosmetics"), icon: Flag, count: shopInv.cosmetics.length },
          { id: "boosts", label: t("inventory.tab.boosts"), icon: Zap, count: shopInv.boosts.length },
          { id: "consumables", label: t("inventory.tab.consumables"), icon: Scroll, count: shopInv.consumables.length },
          { id: "perks", label: t("inventory.tab.perks"), icon: Castle, count: shopInv.perks.length },
          { id: "mounts", label: t("inventory.tab.mounts"), icon: Sparkles, count: shopInv.mounts?.length || 0 },
          { id: "auras", label: t("inventory.tab.auras"), icon: Zap, count: shopInv.auras?.length || 0 },
        ].map((tabItem) => (
          <button key={tabItem.id} onClick={() => setTab(tabItem.id)} data-testid={`inv-tab-${tabItem.id}`}
            className={`px-4 py-2 rounded-md text-sm font-bold font-display tracking-wide border transition-all flex items-center gap-2 ${tab === tabItem.id ? "border-cyan-500/60 text-cyan-300 bg-cyan-500/10 shadow-[0_0_14px_rgba(0,229,255,0.2)]" : "border-white/10 text-zinc-400 hover:border-white/30"}`}>
            <tabItem.icon className="w-3.5 h-3.5" />
            {tabItem.label} <span className="font-mono-stat text-[10px] opacity-70">({tabItem.count})</span>
          </button>
        ))}
        <button
          type="button"
        ...
        >
          <Info className="w-3.5 h-3.5" /> {t("inventory.guide.btn")}
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6 justify-center" data-testid="rarity-filters">
        <button onClick={() => setFilter("all")} className={`px-3 py-1.5 rounded text-xs font-bold font-display tracking-wide border ${filter === "all" ? "border-cyan-500/60 text-cyan-300" : "border-white/10 text-zinc-400"}`}>{t("inventory.filter.all")}</button>
        {localizedRarities.map((r) => {
        ...
        {filtered.length === 0 && (
          <div className="col-span-full">
            <PremiumCard tone="gold" className="p-12 text-center text-zinc-500">
              ...
              <div className="italic">{t("inventory.empty.relics")}</div>
            </PremiumCard>
          </div>
        )}
        {filtered.map((item, i) => {
          ...
          return (
            ...
            >
              ...
              <div className={`text-[8px] uppercase tracking-[0.2em] font-bold mt-1 ${tok.text}`}>
                {localizedRarities.find((x) => x.id === item.rarity)?.name || item.rarity}
              </div>
              {item.quantity > 1 && (
              ...
              <div className="absolute inset-x-0 bottom-0 py-1 bg-black/70 opacity-0 group-hover:opacity-100 group-has-[data-info-open]:opacity-0 group-has-[data-info-open]:pointer-events-none transition-opacity flex items-center justify-center gap-1 text-[8px] uppercase tracking-widest font-bold text-amber-200 rounded-b-xl">
                <Gift className="w-2.5 h-2.5" /> {t("inventory.hover.giftTrade")}
              </div>
            </motion.div>
          );
        })}
      ...
      <PremiumModal open={!!newItems} onClose={() => setNewItems(null)} title={t("inventory.chest.reveal")} icon={Sparkles} maxWidth="max-w-2xl" testid="chest-modal">
        <div className="p-6 text-center">
          <div className="text-[10px] uppercase tracking-[0.4em] text-yellow-400 font-bold mb-2">{t("inventory.chest.sealBroken")}</div>
          <p className="text-zinc-400 mb-6 italic">{t("inventory.chest.intro")}</p>
            ...
            {newItems?.map((it) => {
              ...
              return (
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.15 }}
                  ...
                  <div className={`text-[10px] uppercase tracking-[0.25em] font-bold mt-1 ${r.text}`}>{localizedRarities.find((x) => x.id === it.rarity)?.name || it.rarity}</div>
                </motion.div>
              );
            })}
          </div>
          <PremiumButton variant="cyan" size="sm" onClick={() => setNewItems(null)} testid="close-chest-modal">
            {t("inventory.chest.reseal")}
          </PremiumButton>
        </div>
      </PremiumModal>
...
function InventoryInfoButton({ text, testId, className = "", t }) {
  ...
  const popover = open && createPortal(
    ...
    >
      <div className="text-[10px] uppercase tracking-widest text-cyan-300 font-bold mb-1.5">{t("inventory.info.howToUse")}</div>
    ...
    <>
      ...
      >
        <button
          ...
          data-testid={testId}
          aria-label={t("inventory.info.aria")}
          aria-expanded={open}
...
function InventoryGuideModal({ open, onClose, activeTab }) {
  const { t } = useI18n();
  const tabGuide = useMemo(() => getInventoryTabGuide(t, activeTab), [t, activeTab]);
  const actionsGuide = useMemo(() => getInventoryActionsGuide(t), [t]);
  const tabGuides = useMemo(() => getInventoryTabGuides(t), [t]);
  const sourcesGuide = useMemo(() => getInventorySourcesGuide(t), [t]);
  return (
    <PremiumModal open={open} onClose={onClose} title={t("inventory.guide.title")} icon={Info} maxWidth="max-w-2xl" testid="inventory-guide-modal">
      <div className="p-5 space-y-6 max-h-[70vh] overflow-y-auto text-sm text-zinc-300">
        {tabGuide && (
          <section>
            <h3 className="text-[10px] uppercase tracking-[0.25em] text-cyan-300 font-bold mb-2">
              {t("inventory.guide.currentTab", { title: tabGuide.title })}
            </h3>
        ...
        <section>
          <h3 className="text-[10px] uppercase tracking-[0.25em] text-amber-300 font-bold mb-2">{t("inventory.guide.actionsTitle")}</h3>
          <div className="space-y-2">
            {actionsGuide.map((row) => (
        ...
        <section>
          <h3 className="text-[10px] uppercase tracking-[0.25em] text-violet-300 font-bold mb-2">{t("inventory.guide.tabsTitle")}</h3>
          <div className="space-y-3">
            {Object.values(tabGuides).map((g) => (
        ...
        <section>
          <h3 className="text-[10px] uppercase tracking-[0.25em] text-emerald-300 font-bold mb-2">{t("inventory.guide.sourcesTitle")}</h3>
          <div className="grid sm:grid-cols-2 gap-2">
            {sourcesGuide.map((row) => (
...
function SendEcusModal({ open, onClose, onDone }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      sfx.success();
      toast.success(t("inventory.send.success", { amount, name: form.to_username.trim() }));
      ...
      <form onSubmit={submit} className="p-5 space-y-4">
        <div>
          ...
          <input value={form.to_username} onChange={(e) => setForm({ ...form, to_username: e.target.value })}
            placeholder={t("inventory.send.placeholder")} required minLength={3}
        ...
        <div>
          <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mb-1 block">{t("inventory.send.messageOptional")}</label>
          <input value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })}
            maxLength={200} placeholder={t("inventory.send.note")}
...
function ItemActionModal({ item, onClose, onDone }) {
  ...
  const gift = async (e) => {
    e.preventDefault();
    if (!username.trim()) { toast.error(t("inventory.username.required")); return; }
    setSaving(true);
    try {
      await api.post("/economy/gift-item", { to_username: username.trim(), item_id: relicId, quantity: qty });
      sfx.success();
      toast.success(t("inventory.action.giftSuccess", { item: item.name, name: username.trim() }));
      onClose();
      await onDone();
    } catch (err) {
  ...
  const proposeTrade = async (e) => {
    ...
    try {
      ...
      sfx.success();
      toast.success(t("inventory.trade.sent", { name: username.trim() }));
      onClose();
      await onDone();
    } catch (err) {
      ...
      <div className="p-5 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className={`text-[10px] uppercase tracking-widest font-bold ${tok.text}`}>{item.rarity} · x{maxQty}</div>
          </div>
        </div>
        <p className="text-[11px] text-zinc-400 leading-relaxed border border-cyan-500/15 bg-cyan-500/5 rounded-lg px-3 py-2">
          {relicUsageInfo(item, t)}
        </p>
        ...
        {mode === "menu" && (
          <div className="grid grid-cols-2 gap-3 pt-1">
            <button type="button" onClick={() => setMode("gift")} data-testid="action-gift"
              ...
              <div className="text-sm font-bold text-emerald-200">{t("inventory.action.gift")}</div>
              <div className="text-[10px] text-zinc-500 mt-0.5">{t("inventory.action.giftSub")}</div>
            </button>
            <button type="button" onClick={() => setMode("trade")} data-testid="action-trade"
              ...
              <div className="text-sm font-bold text-amber-200">{t("inventory.action.trade")}</div>
              <div className="text-[10px] text-zinc-500 mt-0.5">{t("inventory.action.tradeSub")}</div>
            </button>
          </div>
        )}
          ...
          <form onSubmit={mode === "gift" ? gift : proposeTrade} className="space-y-3">
            <div>
              ...
              <input value={username} onChange={(e) => setUsername(e.target.value)} required minLength={3}
                placeholder={t("inventory.send.placeholder")}
                className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-sm" data-testid="action-username" />
            </div>
            {maxQty > 1 && (
              <div>
                <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mb-1 block">{t("inventory.action.quantity")}</label>
            ...
            {mode === "trade" && (
              <>
                <div>
                  <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mb-1 block">{t("inventory.action.ecusOptional")}</label>
                  <input type="number" min={0} value={tradeEcus} onChange={(e) => setTradeEcus(e.target.value)}
                    placeholder="0"
                    className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-sm font-mono-stat" />
                </div>
                <div>
                  <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mb-1 block">{t("inventory.action.note")}</label>
                  <input value={note} onChange={(e) => setNote(e.target.value)} maxLength={200}
                    placeholder={t("inventory.action.notePlaceholder")}
                    className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-sm" />
                </div>
                <p className="text-[10px] text-zinc-500 italic">
                  {t("inventory.action.reserveNote")}
                </p>
              </>
            )}
            <div className="flex gap-2">
              <button type="button" onClick={() => setMode("menu")} className="px-3 py-2 text-sm text-zinc-400">{t("inventory.action.back")}</button>
              <PremiumButton type="submit" variant={mode === "gift" ? "cyan" : "gold"} size="md" disabled={saving} className="flex-1"
                ...
                {saving ? "…" : mode === "gift" ? t("inventory.action.gift") : t("inventory.action.proposeTrade")}
              </PremiumButton>
            </div>
          </form>
        )}
...
function TradesModal({ open, onClose, trades, relics, onDone }) {
  ...
  const decline = async (id) => {
    setBusy(true);
    try { await api.post(`/economy/trades/${id}/decline`); toast.info(t("inventory.trades.declined")); await onDone(); }
    catch (e) { toast.error(e.response?.data?.detail || t("settings.error.generic")); }
    finally { setBusy(false); }
  };
  const cancel = async (id) => {
    setBusy(true);
    try { await api.post(`/economy/trades/${id}/cancel`); toast.info(t("inventory.trades.cancelled")); await onDone(); }
    catch (e) { toast.error(e.response?.data?.detail || t("settings.error.generic")); }
    finally { setBusy(false); }
  };
  const accept = async (id) => {
    ...
    try {
      ...
      sfx.success();
      toast.success(t("inventory.trades.completed"));
      resetCounter();
      await onDone();        // recharge l'inventaire/écus immédiatement
      onClose();             // referme le panneau pour révéler l'inventaire à jour
    } catch (e) { toast.error(e.response?.data?.detail || t("inventory.trades.acceptFailed")); }
    finally { setBusy(false); }
  };
    ...
    <div className="text-xs text-zinc-300 space-y-0.5">
      ...
      {trade.expires_at && (
        <div className="text-[10px] text-amber-400/80 font-mono-stat mt-1 flex items-center gap-1">
          <Lucide.Clock className="w-2.5 h-2.5" /> {t("inventory.trades.expiresIn", { time: timeLeft(trade.expires_at) })}
        </div>
      )}
    </div>
  );
      ...
      <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
        <div>
          ...
          {(!trades.incoming || trades.incoming.length === 0) && (
            <div className="text-sm text-zinc-500 italic py-3">{t("inventory.trades.incomingEmpty")}</div>
          )}
          <div className="space-y-3">
            {(trades.incoming || []).map((trade) => (
              <div key={trade.trade_id} className="rounded-xl border border-amber-500/25 bg-amber-500/5 p-3" data-testid={`trade-in-${trade.trade_id}`}>
                <div className="text-sm font-bold text-white mb-1">{t("inventory.trades.proposes", { name: trade.from_username })}</div>
                {renderGoods(trade)}
                {accepting === trade.trade_id ? (
                  <div className="mt-3 border-t border-white/10 pt-3 space-y-2">
                    ...
                    <input type="number" min={0} value={counterEcus} onChange={(e) => setCounterEcus(e.target.value)}
                      placeholder={t("inventory.trades.ecusOptional")}
                      className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-sm font-mono-stat" />
                    <div className="max-h-40 overflow-y-auto space-y-1 pr-1">
                      {relics.length === 0 && <div className="text-xs text-zinc-500 italic">{t("inventory.trades.noRelics")}</div>}
                      {relics.map((r) => {
                  ...
                  <div className="flex gap-2 mt-3">
                    ...
                    <button type="button" onClick={() => decline(trade.trade_id)} disabled={busy}
                      ...
                      <X className="w-3.5 h-3.5 inline mr-1" /> {t("inventory.trades.decline")}
                    </button>
                  </div>
                )}
              </div>
        ...
        <div>
          ...
          {(!trades.outgoing || trades.outgoing.length === 0) && (
            <div className="text-sm text-zinc-500 italic py-3">{t("inventory.trades.outgoingEmpty")}</div>
          )}
          <div className="space-y-3">
            {(trades.outgoing || []).map((trade) => (
              <div key={trade.trade_id} className="rounded-xl border border-violet-500/25 bg-violet-500/5 p-3" data-testid={`trade-out-${trade.trade_id}`}>
                <div className="text-sm font-bold text-white mb-1">{t("inventory.trades.toUser", { name: trade.to_username })}</div>
                {renderGoods(trade)}
  ...
  if (list.length === 0) {
    return (
      <PremiumCard tone="cyan" className="p-12 text-center text-zinc-500 max-w-xl mx-auto" testid={`empty-${tab}`}>
        <Package className="w-12 h-12 mx-auto mb-3 opacity-40" />
        <div className="italic">{t("inventory.shop.emptyCategory")}</div>
      </PremiumCard>
    );
  }
      ...
      {list.map((row, i) => {
        ...
        return (
          <PremiumCard key={`${sku}-${i}`} tone="violet" className={`border-2 relative ${tok.border}`}
            ...
            <InventoryInfoButton
              text={shopOwnedUsageInfo(tab, sku, meta, t)}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className={`text-[9px] uppercase tracking-[0.25em] font-bold mt-0.5 ${tok.text}`}>{meta.rarity}</div>
                {meta.description && (
                ...
                {tab === "boosts" && row.expires_at && (
                  <div className="text-[10px] font-mono-stat text-cyan-300 mt-1.5">
                    {t("inventory.shop.expires", { date: fmtDate(row.expires_at) })}
                  </div>
                )}
                {tab === "cosmetics" && (isFrame || isBanner) && (
                  <PremiumButton variant={equipped ? "cyan" : "ghost"} size="sm" className="mt-2"
                    ...
                    {equipped ? t("inventory.shop.equipped") : t("inventory.shop.equip")}
                  </PremiumButton>
                )}
                {tab === "auras" && (
                  <PremiumButton variant={equipped ? "cyan" : "ghost"} size="sm" className="mt-2"
                    onClick={() => onEquipAura(sku)}>
                    {equipped ? t("inventory.shop.auraActive") : t("inventory.shop.equipAura")}
                  </PremiumButton>
                )}
                {tab === "mounts" && (
                  <PremiumButton variant={equipped ? "cyan" : "ghost"} size="sm" className="mt-2"
                    onClick={() => onEquipMount(sku)}>
                    {equipped ? t("inventory.shop.mountActive") : t("inventory.shop.equipMount")}
                  </PremiumButton>
                )}
              </div>
            </div>
